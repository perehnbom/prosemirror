...
function runFootnote(state, dispatch) {
  let {empty, $from, $to} = state.selection, content = Fragment.empty
  if (!empty && $from.sameParent($to) && $from.parent.inlineContent)
    content = $from.parent.content.cut($from.parentOffset, $to.parentOffset)
  dispatch(state.tr.replaceSelectionWith(footnoteSchema.nodes.footnote.create(null, content)))
...
class FootnoteView {
  constructor(node, view, getPos) {
    ...
    this.innerView = null
    this.tooltip = null
    console.log('construct footnote')
  }
...
function initProsemirror(element, viewModel){
  viewModel.editor = new EditorView(element, {
    state: EditorState.create({
      doc: DOMParser.fromSchema(footnoteSchema).parse(document.querySelector("#default-content")),
...
can.Component.extend({
  ...
  events: {
    ...
    '#fn mousedown' : function(el, ev){
      ev.preventDefault();
      var editor = this.viewModel.editor;
      runFootnote(editor.state, editor.dispatch);
    }
  }
});
